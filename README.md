...
#### User options 
To turn on masking of the second source in the pair, change the variable "mask_second" to True on line 1915 or `calculate_rm.py`. Note, the mask is removed when the RM of the second source is determined. To save the plots produced by the RMAP automatically, change the variable "save_figs_auto" on line 1918 to True. To stop the FITS files of the postage stamps from being saved, change "create_fits" on line 1923 to False. To have the RMAP run without user input (i.e. without verifying the source flags), change the variable "get_user_input" on line 1924 to False. To have the plots produced with a black background, set the variable "light_background" on line 1925 to False. 
