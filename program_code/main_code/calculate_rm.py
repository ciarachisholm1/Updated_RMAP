...
def plot_rm_full3(data, x_long, y_lat, delta, rm_value, drm_value, pa_err, dpa_err, ston):
    ...
    # The following line was changed from the one above by Ciara Chisholm on August 20th 2026
    rm_text = f"{rm_value:.0f}"
    rm_err_text = f"{drm_value:.0f}"
...
def main(input_directory, output_directory, chitable_directory, fig_path):
    ...
    mask_second = False
    print("\nMask the second source when fitting the first in a pair: ", str(mask_second))
    
    save_figs_auto = False
    print("Save plots automatically: ", str(save_figs_auto))
    ...
    if not os.path.exists(sourcelist_path):
        ...
        if do_continue:
            ...
            while i < num_sources: 
                ...
                # Added by Ciara Chisholm on August 20th 2026
                if mask_second:
